...
% 我们使用NVIDIA的Isaac Sim通过IsaacLab框架开发了我们的模拟环境。IsaacLab是构建在Isaac Sim之上的更高级抽象层，提供专为机器人研究和强化学习设计的简化API和工具。虽然Isaac Sim提供基于NVIDIA PhysX的底层高保真物理模拟，但IsaacLab提供了便捷的包装器、预构建的机器人模型和强化学习工具，加速了开发。我们的模拟包括：
...
% 风干扰模型生成大小在0.2至0.4牛顿之间且方向随机变化的风力。风况定期更新以模拟变化的风型，创造了一个具有挑战性的控制环境，需要智能体表现出适应性行为。IsaacLab与Isaac Sim的集成使我们能够高效地实现这些基于物理的功能，同时保持适合强化学习实验的训练速度。
...
% 如表\ref{tab:hyperparameters}所示，每种算法都经过了不同集数的训练，为公平比较提供了充足的学习时间，同时也考虑到了它们不同的样本效率。
